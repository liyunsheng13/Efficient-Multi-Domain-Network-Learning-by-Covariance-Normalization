...
root = '/gpu6_ssd/yunsheng/dataset/features/256_ObjectCategories/'
...
for k in range(args.start, args.end):
    ...
    pca.fit(features)
    pickle.dump(pca, open(osp.join(root, exact_list[k] + '_wh') + '_pca.sav', 'wb'))
    ...
    pca = decomposition.PCA(n_components=dim)
    pca.fit(features)
    pickle.dump(pca, open(osp.join(root, exact_list[k] + '_rc') + '_pca.sav', 'wb'))    
